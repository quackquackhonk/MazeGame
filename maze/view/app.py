...
class MazeApp:
    ...
    def on_event(self, event):
        ...
        if (event.type == pygame.KEYDOWN):
            k = event.key
            if (k == pygame.K_UP and self.grid[loc[1]][loc[0]].up is not False):
                self.player.move((0, -1))
            if (k == pygame.K_DOWN and self.grid[loc[1]][loc[0]].down is not False):
                self.player.move((0, 1))
            if (k == pygame.K_LEFT and self.grid[loc[1]][loc[0]].left is not False):
                self.player.move((-1, 0))
            if (k == pygame.K_RIGHT and self.grid[loc[1]][loc[0]].right is not False):
                self.player.move((1, 0))
        # key presses
    ...
    def on_cleanup(self):
        if self.player.reachedGoal(): print("You did it!")
        pygame.quit()
